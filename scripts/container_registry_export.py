...
def main(subscription_id: str, subscription_name: str) -> utils.ExportResult:
    ...
    for reg in registries:
        ...
        rows.append({
            ...
            "Public Network Access": utils.s(getattr(reg, "public_network_access", None)),
            "Anonymous Pull": "Yes" if getattr(reg, "anonymous_pull_enabled", False) else "No",
            "Zone Redundancy": utils.s(getattr(reg, "zone_redundancy", None)),
            "Provisioning State": utils.s(reg.provisioning_state),
            "Creation Date": utils.s(reg.creation_date),
